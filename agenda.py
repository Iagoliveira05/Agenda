...
## Conexão
def ConexaoBanco():
    caminho = r"C:\Users\Iago\OneDrive\Documentos\01_Codigos\03_bancos\agenda.db"
    con = None
    try:
...
def query(conexao, sql):    # INSERT, DELETE e UPDATE
    try:
        c = conexao.cursor()
        c.execute(sql)
        conexao.commit()
...
def menuAtualizar():
    ...
    remail = r[0][3]
    vnome = str(input("Digite o nome: "))
    vtelefone = str(input("Digiet o telefone: "))
    vemail = str(input("Digite o email: "))
    if (len(vnome) == 0):
        vnome = rnome
    if (len(vtelefone) == 0):
